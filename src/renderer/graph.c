...
void DrawGraph(float displacement, float time, SimColor themeColor) {
    ...
    // X-axis (time)
    DrawLine(offsetX + MARGIN, offsetY + GRAPH_HEIGHT - MARGIN, offsetX + GRAPH_WIDTH - MARGIN, offsetY + GRAPH_HEIGHT - MARGIN, BLACK);
    ...
    if (pointCount > 1) {
        ...
        float displacementRange = maxDisplacement - minDisplacement;
        if (displacementRange < 0.1f) displacementRange = 0.1f; // Avoid division by zero
        
        for (int i = 0; i < pointCount - 1; i++) {
            // Only draw points within the time window
            if (dataPoints[i].x < timeWindowStart && dataPoints[i + 1].x < timeWindowStart) continue;
            if (dataPoints[i].x > timeWindowEnd) break;
        ...
        float y = offsetY + GRAPH_HEIGHT - MARGIN - ((dataPoints[pointCount - 1].y - minDisplacement) / displacementRange) * graphHeight;
    ...
    DrawText(TextFormat("%.1f", time), offsetX + GRAPH_WIDTH - MARGIN - 20, offsetY + GRAPH_HEIGHT - MARGIN + 5, 12, GRAY);
}
